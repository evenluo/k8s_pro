...
> 📌 **概念卡：核心课程三模块（Architecture → Components → Deployment）**  
> **定义**：核心课程以“理解架构 → 熟悉对象 → 实践部署”路径组织学习，确保认知从宏观到微观逐层深化。  
> **学习提醒**：每完成一章，回到任务列表核对对应的实验与评估是否完成，避免概念与实践脱节。  
> **关联任务**：T012-T019

> 📌 **概念卡：声明式配置（Declarative Configuration）**  
> **定义**：通过 YAML Manifest 描述期望状态，由 Kubernetes 控制器自动调谐至该状态。  
> **学习提醒**：优先使用 `kubectl apply` 与 Git 管理配置，保留版本历史以支持回滚。  
> **关键命令**：`kubectl apply -f`、`kubectl diff -f`

> 📌 **概念卡：实验闭环（Lab Validation Loop）**  
> **定义**：每个实验以“准备 → 执行 → 验证 → 复盘”四步完成，确保概念落地。  
> **学习提醒**：验证环节建议记录 `kubectl get` 与 `kubectl describe` 的输出，作为后续排障素材。  
> **应用位置**：`courses/01-core/labs/`

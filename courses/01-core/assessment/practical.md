...
> 📌 **概念卡：Three-Tier Application（多层架构）**  
> **定义**：由前端、API、数据层组成的经典架构模式，强调水平扩展与职责分离。  
> **评估作用**：通过分别部署不同层级的 Deployment 与 Service，验证对依赖链路与配置管理的掌握。  
> **验收提示**：使用 `kubectl get deploy -n webapp-prod` 确认各层副本数达标。

> 📌 **概念卡：ResourceQuota（资源配额）**  
> **定义**：在命名空间级别限制资源使用量的对象，防止某一团队消耗全部集群资源。  
> **评估作用**：强制要求设置 CPU/内存/Pod 限额，检验在生产环境中进行资源治理的能力。  
> **关键命令**：`kubectl describe resourcequota -n webapp-prod`

> 📌 **概念卡：Service Topology（服务拓扑）**  
> **定义**：通过 Service 与 Ingress/NodePort 组合，为多层应用建立统一的流量入口。  
> **评估作用**：要求设计访问路径（用户 → Ingress → Service → Pod），确保健康检查与回滚策略完整。  
> **验收提示**：记录 `kubectl get svc -n webapp-prod` 输出并验证端口映射。

> 📌 **概念卡：Operational Runbook（运维手册）**  
> **定义**：对操作步骤、问题排查与恢复流程的文档化描述，确保应急响应可执行。  
> **评估作用**：在故障排查环节要求记录日志命令、定位思路，为后续高级课程的 SRE 实践铺垫。  
> **建议输出**：`问题 → 诊断命令 → 结论 → 修复动作`
